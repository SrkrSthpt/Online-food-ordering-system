...
<section class="hero">
  ...
  <div class="hero-overlay">
    ...
    <div class="hero-search">
      ...
      <button onclick="window.location.href='<?= e(route('menu.index')) ?>'"><i class="fas fa-search"></i> Search</button>
    </div>
  </div>
</section>
    ...
    <div class="restaurant-grid">
      <?php if (count($restaurants) > 0) : ?>
            <?php foreach ($restaurants as $r) : ?>
                ...
                ?>
          <div class="restaurant-card" onclick="window.location.href='<?= e(route('menu.index')) ?>?restaurant=<?= (int)$r->id ?>'">
            <div class="restaurant-card-img" style="background-image: url('<?= e($img) ?>')">
              <div class="overlay"></div>
              <div class="rating"><i class="fas fa-star"></i> <?= number_format((float)$r->rating, 1) ?></div>
            </div>
            <div class="restaurant-card-body">
              <h3><?= e($r->name) ?></h3>
              <p><i class="fas fa-map-marker-alt"></i> <?= e((string)$r->location) ?></p>
              <p style="color: var(--gray); font-size:0.8rem;"><i class="fas fa-utensils"></i> <?= (int)($counts[$r->id] ?? 0) ?> items</p>
              <a href="<?= e(route('menu.index')) ?>?restaurant=<?= (int)$r->id ?>" class="btn">View Menu <i class="fas fa-arrow-right"></i></a>
            </div>
          </div>
            <?php endforeach; ?>
      <?php else : ?>
        <p style="text-align:center;color:var(--gray);grid-column:1/-1;">No restaurants available yet.</p>
      <?php endif; ?>
    </div>
  </div>
</section>
...
function filterBySearch(e) {
  if (e.key === 'Enter' || e.type === 'click' || e.type === 'keyup') {
    window.location.href = '<?= e(route('menu.index')) ?>?q=' + encodeURIComponent(document.getElementById('searchInput').value);
  }
}
</script>